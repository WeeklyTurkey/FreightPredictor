...
def generate_freight_rates():
    ...
    Uses a momentum-based random walk with:
      ...
      - Seasonal variation (monsoon / winter peaks)
      - Occasional market shocks (~5% of days)
      - Very low daily noise (realistic for weekly-settled freight rates)
    """
    import math
    random.seed(42)
    today = date.today()
    ...
    for origin in ORIGIN_PORTS:
        for dest in DESTINATION_PORTS:
            ...
            transit_days = max(5, distance // 350)
            ...
            for vessel in VESSELS:
                for commodity in COMMODITIES:
                    ...
                    mean_rate = base_rate * distance_factor
                    ...
                    while current_date <= today:
                        ...
                        # Seasonal factor: +/- 6% over the year
                        seasonal = 1.0 + 0.06 * math.sin(2 * math.pi * (day_of_year - 80) / 365)

                        # Mean-reversion pull
                        reversion = (mean_rate * seasonal - current_rate) * 0.008

                        # Momentum update: slow-changing trend with autocorrelation
                        momentum = 0.92 * momentum + reversion + random.gauss(0, mean_rate * 0.002)

                        # Occasional market shock (~5% of days)
                        if random.random() < 0.05:
                            shock = random.choice([-1, 1]) * random.uniform(0.03, 0.08) * current_rate
                            momentum += shock * 0.3

                        current_rate = max(mean_rate * 0.6, current_rate + momentum)

                        # Tiny daily micro-noise
                        daily_rate = round(current_rate + random.gauss(0, current_rate * 0.003), 2)
