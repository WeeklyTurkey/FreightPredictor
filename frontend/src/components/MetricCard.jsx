...
export default function MetricCard({ icon: Icon, label, value, unit, change, changePct, trend, accent = 'teal', subtitle }) {
 ...
 className={`p-2.5 ${colors.bg} ${colors.border} border cursor-pointer hover:ring-2 ring-offset-2 transition-all ${accent === 'teal' ? 'ring-teal-200' : accent === 'emerald' ? 'ring-emerald-200' : accent === 'amber' ? 'ring-amber-200' : accent === 'rose' ? 'ring-rose-200' : 'ring-slate-200'}`}
