...
export const runSimulation = async (volumeMt, laycanWeeks, routeId, charterType) => {
  ...
  try {
    ...
    // Base freight rate from the recommendation or fallback
    const baseRate = 24.50;
    const avgTransitDays = 14; // approximate
    const transitDays = avgTransitDays + laycanWeeks * 7;

    // Spot: per-MT freight rate (includes shipowner's vessel cost) + demurrage
    const spotDemurragePremium = Math.max(0, (laycanWeeks - 2) * 0.40);
    const spotRate = (baseRate * 1.03) + spotDemurragePremium;
    const spotTotal = volumeMt * spotRate;

    // Time Charter: hire the whole vessel — no separate per-MT freight charge
    const dailyHireRate = 28000;
    const dailyConsumptionMt = 55;
    const bunkerPricePerMt = 612;
    const portCharges = 45000 * 2;
    const hireCost = dailyHireRate * transitDays;
    const bunkerCost = dailyConsumptionMt * bunkerPricePerMt * transitDays;
    const tcTotal = hireCost + bunkerCost + portCharges;

    const savings = spotTotal - tcTotal;
    const refCost = Math.max(spotTotal, tcTotal);
    const savingsPct = refCost > 0 ? Math.round(Math.abs(savings / refCost) * 1000) / 10 : 0;
...
export const getMarketIndexHistory = async (indexType = 'BDI', days = 90) => {
  if (indexType === 'BDI') return getBdiHistory(days);
  return [];
};

export const getBunkerFuelHistory = async (days = 90) => getVlsfoHistory(days);
